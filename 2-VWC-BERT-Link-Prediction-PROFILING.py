...
writer = SummaryWriter('./tb_logs/link_pred')
...
def train_model(dataloader, c_dataloader, v_dataloader=None, t_dataloader=None):    
    ...
    #tb_handler = torch.profiler.tensorboard_trace_handler('./tb_logs/link_pred')
    
    with torch.profiler.profile(
        #schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=2),        
        schedule=torch.profiler.schedule(skip_first=1, wait=1, warmup=1, active=3, repeat=2),
        #activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        activities=[ProfilerActivity.CUDA],
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./tb_logs/link_pred'),
        record_shapes=True,
        profile_memory=True,
        with_stack=True
) as prof:
        ...
        for epoch_i in range(args.epochs):
            ...
            for step, batch in enumerate(dataloader):
                ...
                print("stuck start: ",step)
                prof.step()
                print("stuck done: ",step)
            ...
            writer.add_scalar('F-1 score',avg_train_acc, epoch_i)
            writer.add_scalar('step time',total_step_time, epoch_i)
